<script setup lang="ts">
  ...
  const { data, error } = await useFetch<Recipes>('https://dummyjson.com/recipes?limit=15')
</script>
    ...
    <section class="bg-[#f1f1f1]">
      <div class="container flex flex-col lg:flex-row items-center py-20 gap-10">
        <div class="flex-1 order-2 lg:order-1 text-center lg:text-left">
          ...
          <button
            class="px-4 py-2 text-white self-start bg-yellow-600 rounded-md text-lg cursor-pointer"
          >
            Browse Recipes
          </button>
        </div>
