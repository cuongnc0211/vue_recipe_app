      ...
      <ul class="grid grid-cols-1 md:grid-cols-2 gap-2 text-lg">
        <li v-for="ingredient in data?.ingredients">
          <label class="flex gap-2 items-center">
            ...
            ></div>
            <span class="peer-checked:line-through peer-checked:text-sky-500">
              {{ ingredient }}
            </span>
          </label>
        </li>
    ...
    <div>
      ...
      <ul class="flex flex-col text-lg gap-4">
        <li v-for="(instruction, index) in data?.instructions" class="flex gap-2">
          <span
            class="flex items-center justify-center bg-yellow-600 w-7 h-7 rounded-full text-white text-sm"
          >
            {{ index + 1 }}
          </span>
